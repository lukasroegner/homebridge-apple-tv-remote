...
function AppleTvPlatform(log, config, api) {
  ...
  platform.api.on('didFinishLaunching', function () {
    ...
    appletv.scan(null, platform.config.scanTimeout).then(function (appleTvs) {
      platform.log('Scanning for AppleTVs completed. ' + appleTvs.length + ' device(s) found.');
      ...
      for (let i = 0; i < platform.config.devices.length; i++) {
        ...
        const foundAppleTvs = appleTvs.filter(function (a) { return a.uid === credentials.uniqueIdentifier; });
        if (foundAppleTvs.length > 0) {
          ...
          promises.push(foundAppleTvs[0].openConnection(credentials).then(function (appleTv) {
